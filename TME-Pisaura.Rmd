...
# TODO: maybe sort out values where Ethovision data does not fit to manual data?
# TODO: maybe sort out unreasonable values of velocity
unique(dt$Zone)
dt <- dt %>%
...
#here select all variables included in long format:
vars <- c('id','boldness','activity','week','t_on_C','t_on_B','Highly_mobile','Immobile','Mobile','Mobility_Mean_%')
dtl <- dt %>%
  select(vars) %>%
  melt(id.vars = c('id','week'), measure.vars=c('boldness','activity','t_on_C')) %>%
  arrange(id)
```
