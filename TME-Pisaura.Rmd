---
title: "TME-Pisaura"
output: html_document
editor_options:
  chunk_output_type: console
---


## Packages

```{r packages}
require(ggplot2)
require(tidyr)
require(data.table)
require(dplyr)
```

## Harvest data

```{r data-harvest}
#read table
TME = fread("TME_Pisaura_data.csv", header = TRUE, encoding = 'Latin-1', stringsAsFactors = FALSE)

```

## Prepare data

```{r data-preparation}
#TODO maybe omit spiders that died during testing / NA values
TMEnoNA = na.omit(TME)
names(TME)[names(TME) == "t_on_start side"] <- "t_on_start_side"
names(TMEnoNA)[names(TMEnoNA) == "t_on_start side"] <- "t_on_start_side"
gsub('c', 'C', TME$side_start)
gsub('c', 'C', TMEnoNA$side_start)
```

## Manipulate data

```{r data-manipulation}
#example for mean time on side for groups of different boldness levels
#cur_number() chooses intervals of equal number of observations
mn_time_side = TMEnoNA %>%
  mutate(boldness_groups = cut_number(boldness, n = 10)) %>%
  group_by(boldness_groups) %>%
  summarise(mean(t_on_B), mean(t_on_C))

mn_side_start = TMEnoNA %>%
  group_by(side_start) %>%
  summarise(mean(t_on_start_side))

#linear regression (which is nonsense) for time on side in relation to spider individuum
lm_side_spider = lm(formula = t_on_B ~ id, data = TME)
summary(lm_side_spider)

#relation between boldness and activity
lm_boldness_activity = lm(formula = boldness ~ activity, data = TME)
summary(lm_boldness_activity)
```
